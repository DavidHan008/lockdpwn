...
{
	Vector3d sigma = Vector3d(100.+_DEG2RAD *gyro[0], 100.+_DEG2RAD *gyro[1], 100.+_DEG2RAD *gyro[2]);
	...
	_P_predict = _P + C1_inv * Matrix3d(Vector3d(sigma[0]*sigma[0], sigma[1]*sigma[1], sigma[2]*sigma[2]).asDiagonal()) * C1_inv.transpose() * dt * dt;
	//cout << Matrix3d(Vector3d(sigma[0], sigma[1], sigma[2]).asDiagonal()) << endl << endl << endl;
	//puts("");
}

void AttitudeKF::Update (double yaw_gps, double vel_gps)	
{
	if(_VEL_MIN < vel_gps)
	{
		...
		double sigma_psi = 0.001;//2 * sigma_gps/(epsilon + vel_gps*2.0);
		
		//sigma_psi =  2.0;
		//sigma_psi = 0.001; //0.001
