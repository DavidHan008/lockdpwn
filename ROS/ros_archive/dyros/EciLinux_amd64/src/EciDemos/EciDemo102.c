...
ECI_RESULT EciCanDemo102  ( DWORD   dwHwIndex,
                            DWORD   dwCtrlIndex);
...
///////////////////////////////////////////////////////////////////////////////
/**
  ECI Demo for iPC-I 320 / ISA (104)
...
ECI_RESULT EciDemo102(void)
{
  ...
  {
    astcHwPara[dwIndex].wHardwareClass = ECI_HW_ISA;
    #ifdef ECIDEMO_HWUSEPOLLINGMODE
      astcHwPara[dwIndex].dwFlags = ECI_SETTINGS_FLAG_POLLING_MODE;
    #else
      astcHwPara[dwIndex].u.sIsaSettings.dwVer            = ECI_STRUCT_VERSION_V0;
      astcHwPara[dwIndex].u.sIsaSettings.u.V0.dwAddress   = 0x0000; //TODO: Enter ISA Address here
      astcHwPara[dwIndex].u.sIsaSettings.u.V0.bInterrupt  = 0;      //TODO: Enter ISA Interrupt here
    #endif //ECIDEMO_HWUSEPOLLINGMODE
  }

  //*** At first call Initialize to prepare ECI driver
  hResult = ECI102_Initialize(_countof(astcHwPara), astcHwPara);
  ECIDEMO_CHECKERROR(ECI102_Initialize);

  //*** Retrieve hardware info
  if(ECI_OK == hResult)
  {
    //*** Retrieve hardware info
    hResult = ECI102_GetInfo(dwHwIndex, &stcHwInfo);
    ECIDEMO_CHECKERROR(ECI102_GetInfo);
    if(ECI_OK == hResult)
  ...
  if(ECI_OK == hResult)
  {
    ...
    {
      //*** Start CAN Demo
      hResult =  EciCanDemo102(dwHwIndex, dwCtrlIndex);
      ECIDEMO_CHECKERROR(EciCanDemo102);
    }
    else
    {
  ...
  OS_Printf("-> Returning from ECI Demo for iPC-I 320 / ISA (104) <-\n");
                         ...
                         DWORD dwCtrlIndex)
{
  ...
  if(ECI_OK == hResult)
  {
    ...
    hResult = ECI102_CtrlOpen(&dwCtrlHandle, dwHwIndex, dwCtrlIndex, &stcCtrlConfig);
    ECIDEMO_CHECKERROR(ECI102_CtrlOpen);
  }
  ...
  if(ECI_OK == hResult)
  {
    ...
    hResult = ECI102_CtrlGetCapabilities(dwCtrlHandle, &stcCtrlCaps);
    ECIDEMO_CHECKERROR(ECI102_CtrlGetCapabilities);
    if(ECI_OK == hResult)
    {
  ...
  if(ECI_OK == hResult)
  {
    hResult = ECI102_CtrlStart(dwCtrlHandle);
    ECIDEMO_CHECKERROR(ECI102_CtrlStart);
  }
  ...
  if(ECI_OK == hResult)
  {
    ...
    for(dwIndex=0; dwIndex < dwTxMsgCount; dwIndex++)
    {
      ...
      #ifdef ECIDEMO_HWUSEPOLLINGMODE
      {
        ...
        do
        {
          hResult = ECI102_CtrlSend( dwCtrlHandle, &stcCtrlMsg, 0);
          if(ECI_OK != hResult)
            { OS_Sleep(1);}
        }while((ECI_OK != hResult) && ((OS_GetTimeInMs() - dwStartTime) < ECIDEMO_TX_TIMEOUT));
      }
      #else
        hResult = ECI102_CtrlSend( dwCtrlHandle, &stcCtrlMsg, ECIDEMO_TX_TIMEOUT);
      #endif //ECIDEMO_HWUSEPOLLINGMODE
      if(ECI_OK != hResult)
      {
        OS_Printf("Error while sending CAN Messages\n");
        ECIDEMO_CHECKERROR(ECI102_CtrlSend);
        hResult = ECI_OK;
        break;
      }
      else
      {
        ...
        hResult = ECI102_CtrlReceive( dwCtrlHandle, &dwCount, astcCtrlMsg, 0);
  ...
  if(ECI_OK == hResult)
  {
    ...
    while(dwRxTimeout >= (dwCurrentTime - dwStartTime))
    {
      ...
      #ifdef ECIDEMO_HWUSEPOLLINGMODE
      {
        ...
        {
          dwCount = 1;
          hResult = ECI102_CtrlReceive( dwCtrlHandle, &dwCount, &stcCtrlMsg, 0);
      ...
      {
        dwCount = 1;
        hResult = ECI102_CtrlReceive( dwCtrlHandle, &dwCount, &stcCtrlMsg, ECIDEMO_RX_TIMEOUT);
      }
      #endif //ECIDEMO_HWUSEPOLLINGMODE
  ...
  if(ECI_OK == hResult)
  {
    hResult = ECI102_CtrlStop(dwCtrlHandle, ECI_STOP_FLAG_NONE);
    ECIDEMO_CHECKERROR(ECI102_CtrlStop);
  }
  ...
  if(ECI_OK == hResult)
  {
    hResult = ECI102_CtrlStop(dwCtrlHandle, ECI_STOP_FLAG_RESET_CTRL);
    ECIDEMO_CHECKERROR(ECI102_CtrlStop);
  }

  //*** Close ECI Controller
  ECI102_CtrlClose(dwCtrlHandle);
