...
ECI_RESULT EciCanDemo105  ( DWORD   dwHwIndex,
                            DWORD   dwCtrlIndex);
ECI_RESULT EciLinDemo105  ( DWORD   dwHwIndex,
                            DWORD   dwCtrlIndex);
...
ECI_RESULT EciDemo105(void)
{
  ...
  if(ECI_OK == hResult)
  {
    ...
    ECIDEMO_CHECKERROR(ECI105_GetInfo);
    if(ECI_OK == hResult)
  ...
  if(ECI_OK == hResult)
  {
    ...
    {
      //*** Start CAN Demo
      hResult =  EciCanDemo105(dwHwIndex, dwCtrlIndex);
      ECIDEMO_CHECKERROR(EciCanDemo105);
    }
    else
    {
  ...
  if(ECI_OK == hResult)
  {
    ...
    {
      //*** Start LIN Demo
      hResult =  EciLinDemo105(dwHwIndex, dwCtrlIndex);
      ECIDEMO_CHECKERROR(EciLinDemo105);
    }
    else
    {
      //*** Ignore if no controller was found
  ...
  OS_Printf("-> Returning from ECI Demo for iPC-I XC16 / PCI (PMC) <-\n");
                         ...
                         DWORD dwCtrlIndex)
{
  ...
  if(ECI_OK == hResult)
  {
    ...
    hResult = ECI105_CtrlOpen(&dwCtrlHandle, dwHwIndex, dwCtrlIndex, &stcCtrlConfig);
  ...
  if(ECI_OK == hResult)
  {
    ...
    hResult = ECI105_CtrlGetCapabilities(dwCtrlHandle, &stcCtrlCaps);
    ECIDEMO_CHECKERROR(ECI105_CtrlGetCapabilities);
    if(ECI_OK == hResult)
    {
  ...
  if(ECI_OK == hResult)
  {
    ...
    for(dwIndex=0; dwIndex < dwTxMsgCount; dwIndex++)
    {
      ...
      #ifdef ECIDEMO_HWUSEPOLLINGMODE
      {
        ...
        do
        {
          hResult = ECI105_CtrlSend( dwCtrlHandle, &stcCtrlMsg, 0);
          if(ECI_OK != hResult)
            { OS_Sleep(1);}
        }while((ECI_OK != hResult) && ((OS_GetTimeInMs() - dwStartTime) < ECIDEMO_TX_TIMEOUT));
      }
      #else
        hResult = ECI105_CtrlSend( dwCtrlHandle, &stcCtrlMsg, ECIDEMO_TX_TIMEOUT);
      #endif //ECIDEMO_HWUSEPOLLINGMODE
      if(ECI_OK != hResult)
      {
        OS_Printf("Error while sending CAN Messages\n");
        ECIDEMO_CHECKERROR(ECI105_CtrlSend);
        hResult = ECI_OK;
        break;
      }
      else
      {
        ...
        hResult = ECI105_CtrlReceive( dwCtrlHandle, &dwCount, astcCtrlMsg, 0);
  ...
  if(ECI_OK == hResult)
  {
    ...
    while(dwRxTimeout >= (dwCurrentTime - dwStartTime))
    {
      ...
      #ifdef ECIDEMO_HWUSEPOLLINGMODE
      {
        ...
        {
          dwCount = 1;
          hResult = ECI105_CtrlReceive( dwCtrlHandle, &dwCount, &stcCtrlMsg, 0);
      ...
      {
        dwCount = 1;
        hResult = ECI105_CtrlReceive( dwCtrlHandle, &dwCount, &stcCtrlMsg, ECIDEMO_RX_TIMEOUT);
      }
      #endif //ECIDEMO_HWUSEPOLLINGMODE
