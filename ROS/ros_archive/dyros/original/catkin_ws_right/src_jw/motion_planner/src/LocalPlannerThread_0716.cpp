...
void LocalPlannerThread::GetLookAheadPt_JW(double &lookAheadDist,double& x, double& y, double &resdist)
{
	...
	//JW 16.06.24.test1
	//if(m_pos[2] > M_PI)	heading = m_pos[2] - M_PI*2;
	//else	heading = m_pos[2];
	heading = m_pos[2];
	...
	//m_th_Err = 10*(1-m_th_Err)*(1-m_th_Err)*(1-m_th_Err)*(1-m_th_Err);
...
//	delta_LookAhead = m_th_Err ;//+ turning_rad*m_th_Err*m_dist_Err;// + 0.1*1*m_th_Err*m_dist_Err;
	delta_LookAhead = 10*(float)((1-m_th_Err)*(1-m_th_Err)*(1-m_th_Err)*(1-m_th_Err)*(1-m_th_Err)) + turning_rad*m_th_Err*m_dist_Err;// + 0.1*1*m_th_Err*m_dist_Err;
	...
	cout << "th_err : " << m_th_Err << ", dist_err : " << m_dist_Err << ", rad : " << turning_rad << endl;
	cout << "JoonWoo Delta : " << lookAheadDist << ", TH_ERR : " << 10*(1-sin(m_th_Err*M_PI/2)) << ", Dist_ERR : "<<  (turning_rad*m_th_Err*m_dist_Err) << endl<< endl;
