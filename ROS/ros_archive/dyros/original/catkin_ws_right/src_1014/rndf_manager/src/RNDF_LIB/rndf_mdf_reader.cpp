 ...
 */
#include "../../inc/RNDF_LIB/rndf_mdf_reader.h"
#include <iostream>
...
struct waypoint *createWaypoint(char *line)
{
  ...
  wp = (waypoint*)malloc(sizeof(struct waypoint));
  assert(wp != 0);
...
struct checkpoint *createCheckpoint(char *line)
{
  ...
  cp = (checkpoint*)malloc(sizeof(struct checkpoint));
  assert(cp != 0);
  now = numberOfWords(line);
  assert(now == 3);
  params = divideLine(line, now, MAX_STRING_LENGTH);
 ...
 */
struct exit_ *createExit(char *line)
{
  char **params;
  struct exit_ *e;
  int now;
  e = (exit_*)malloc(sizeof(struct exit_));
  assert(e != 0);
  now = numberOfWords(line);
  assert(now == 3);
  params = divideLine(line, now, MAX_STRING_LENGTH);
...
struct stop *createStop(char *line)
{
  ...
  s = (stop*)malloc(sizeof(struct stop));
  assert(s != 0);
  now = numberOfWords(line);
  assert(now == 2);
  params = divideLine(line, now, MAX_STRING_LENGTH);
...
struct lane *createLane(char **p)
{
  ...
  l = (lane*)malloc(sizeof(struct lane));
  assert( l != 0);
  ...
  temp =(char*)( "left_boundary");
  now = numberOfWords(p[n]);
  params = divideLine(p[n], now, MAX_STRING_LENGTH);
  assert(params != 0);
  if(compTwoStrings(params[0], temp)){
    assert(now == 2);
    l -> left_boundary = (char*)malloc(MAX_STRING_LENGTH * sizeof(char));
  ...
  temp =(char*)( "right_boundary");
  now = numberOfWords(p[n]);
  params = divideLine(p[n], now, MAX_STRING_LENGTH);
  assert(params != 0);
  if(compTwoStrings(params[0], temp)){
    assert(now == 2);
    l -> right_boundary = (char*)malloc(MAX_STRING_LENGTH * sizeof(char));
  ...
  if(numOfCkPts > 0)
    l -> checkpoints_list = (checkpoint **)malloc(numOfCkPts * sizeof(struct checkpoint));
  else
  ...
  if(numOfSps > 0)
    l -> stops_list = (stop **)malloc(numOfSps * sizeof(struct stop));
  else
    l -> stops_list = 0;
  ...
  if(numOfEts > 0)
    l -> exits_list = (exit_**)malloc(numOfEts * sizeof(struct stop));
  else
    l -> exits_list = 0;
  ...
  l -> waypoints_list = (waypoint **)malloc(
...
struct segment *createSegment(char **p)
{
  ...
  s = (segment*)malloc(sizeof(struct segment));
  assert(s != 0);
  now = numberOfWords(p[n]);
  assert(now == 2);
  params = divideLine(p[n], now, MAX_STRING_LENGTH);
  ...
  temp =(char*)( "segment_name");
  now = numberOfWords(p[n]);
  params = divideLine(p[n], now, MAX_STRING_LENGTH);
  assert(params != 0);
  if(compTwoStrings(params[0], temp)){
    assert(now == 2);
    s -> segment_name = (char*)malloc(MAX_STRING_LENGTH * sizeof(char));
  ...
  s -> lanes_list = (lane**)malloc(s -> number_of_lanes * sizeof(struct lane));
  lane_start = n;
  lane_end = n + 1;
  temp =(char*)( "end_lane");
...
struct perimeterpoint *createPerimeterpoint(char *line)
{
  ...
  pp = (perimeterpoint*)malloc(sizeof(struct perimeterpoint));
  assert(pp != 0);
  now = numberOfWords(line);
  assert(now == 3);
  params = divideLine(line, now, MAX_STRING_LENGTH);
...
struct perimeter *createPerimeter(char **p)
{
  ...
  p_s = (perimeter*)malloc(sizeof(struct perimeter));
  assert(p_s != 0);
  now = numberOfWords(p[n]);
  assert(now == 2);
  params = divideLine(p[n], now, MAX_STRING_LENGTH);
  ...
  if(numOfEts > 0)
    p_s -> exits_list = (exit_**)malloc(numOfEts * sizeof(struct stop));
  else
    p_s -> exits_list = 0;
  for(i = 0; i < numOfEts; i++){
  ...
  p_s -> perimeterpoints_list = (perimeterpoint**)malloc(
...
struct spot *createSpot(char **p)
{
  ...
  s_s = (spot*)malloc(sizeof(struct spot));
  assert(s_s != 0);
  now = numberOfWords(p[n]);
  assert(now == 2);
  params = divideLine(p[n], now, MAX_STRING_LENGTH);
...
struct zone *createZone(char **p)
{
  ...
  z = (zone*)malloc(sizeof(struct zone));
  assert(z != 0);
  now = numberOfWords(p[n]);
  assert(now == 2);
  params = divideLine(p[n], now, MAX_STRING_LENGTH);
  ...
  if(compTwoStrings(params[0], temp)){
    assert(now == 2);
    z -> zone_name = (char*)malloc(MAX_STRING_LENGTH * sizeof(char));
  ...
  z -> parking_spots_list = 
    (spot**)malloc(z -> number_of_parking_spots * sizeof(struct zone));
  s_start = n;
  s_end = n + 1;
  temp =(char*)( "end_spot");
...
struct RNDF *createRNDF(char **p)
{
  ...
  r = (RNDF*)malloc(sizeof(struct RNDF));
  assert(r != 0);
  now = numberOfWords(p[n]);
  assert(now == 2);
  params = divideLine(p[n], now, MAX_STRING_LENGTH);
  assert(params != 0);
  r -> RNDF_name = (char*)malloc(MAX_STRING_LENGTH * sizeof(char));
  ...
  temp =(char*)( "format_version");
  now = numberOfWords(p[n]);
  params = divideLine(p[n], now, MAX_STRING_LENGTH);
  assert(params != 0);
  if(compTwoStrings(params[0], temp)){
    assert(now == 2);
    r -> format_version = (char*)malloc(MAX_STRING_LENGTH * sizeof(char));
  ...
  temp =(char*)( "creation_date");
  now = numberOfWords(p[n]);
  params = divideLine(p[n], now, MAX_STRING_LENGTH);
  assert(params != 0);
  if(compTwoStrings(params[0], temp)){
    assert(now == 2);
    r ->  creation_date = (char*)malloc(MAX_STRING_LENGTH * sizeof(char));
  ...
  r -> segments_list = 
    (segment**)malloc(r -> number_of_segments * sizeof(struct segment));
  s_start = n;
  s_end = n + 1;
  temp =(char*)( "end_segment");
  ...
  r -> zones_list = 
   (zone**)malloc(r -> number_of_zones * sizeof(struct zone));
  z_start = n;
  z_end = n + 1;
  temp =(char*)( "end_zone");
...
struct speed_limit *createSpeed_limit(char *line)
{
  ...
  sl = (speed_limit*)malloc(sizeof(struct speed_limit));
  assert(sl != 0);
  now = numberOfWords(line);
  assert(now == 3);
  params = divideLine(line, now, MAX_STRING_LENGTH);
...
struct MDF *createMDF(char **p)
{
  ...
  m = (MDF*)malloc(sizeof(struct MDF));
  assert(m != 0);
  now = numberOfWords(p[n]);
  assert(now == 2);
  params = divideLine(p[n], now, MAX_STRING_LENGTH);
  assert(params != 0);
  m -> MDF_name = (char*)malloc(MAX_STRING_LENGTH * sizeof(char));
  ...
  params = divideLine(p[n], now, MAX_STRING_LENGTH);
  assert(params != 0);
  m -> RNDF_name = (char*)malloc(MAX_STRING_LENGTH * sizeof(char));
  ...
  if(compTwoStrings(params[0], temp)){
    assert(now == 2);
    m -> format_version = (char*)malloc(MAX_STRING_LENGTH * sizeof(char));
  ...
  if(compTwoStrings(params[0], temp)){
    assert(now == 2);
    m -> creation_date = (char*)malloc(MAX_STRING_LENGTH * sizeof(char));
  ...
  m -> checkpoints_list = (int*)malloc((m -> number_of_checkpoints) * sizeof(int));
  ...
  m -> speed_limits_list = (speed_limit**)malloc((m -> number_of_speed_limits) * 
...
struct RNDF *parseAnalyzeRndfFile(FILE *file)
{
  ...
  rewind(file);
  arr2 = getArrayOfLines(file, max_nol, 1000);
