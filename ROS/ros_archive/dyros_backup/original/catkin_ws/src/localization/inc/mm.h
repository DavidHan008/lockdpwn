...
#define RAND_MAX_W 32767


inline double DeltaRad (double ang1, double ang2)//ang 1 = -GPS_Heading, ang 2 = Psi(Counter clockwise +180deg ~ clockwise -180deg)
{
	double da = ang1 - ang2;
	if (-M_PI < da && da < M_PI) return da;	//-180deg~+180deg
	else {
		da = fmod (da, 2*M_PI);
...
/*Heading Test
	double da = ang1 - ang2;
	if(0< da && da < 2*M_PI) return da;
	else{
		da = fmod(da,2*M_PI); //360 에대한 제거 
		if(2*M_PI <= da) return da - 2*M_PI;
		else if(da <= 0) return da + 2*M_PI;
		else return da;
		}
	return da;
	*/
...
inline double GaussRand_()
{
    ...
    if (phase == 0) {
		do {
			double U1 = (double)rand() / RAND_MAX_W;
			double U2 = (double)rand() / RAND_MAX_W;
...
{
	static double gr[RAND_MAX_W] = { -1., };

	if (gr[0] == -1.) {
		for (int i=0; i<RAND_MAX_W; ++i) {
			gr[i] = GaussRand_();
		}
	}
	return gr[rand()];
