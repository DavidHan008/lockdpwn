...
HeightMap::HeightMap(ros::NodeHandle node, ros::NodeHandle priv_nh){
  ...
  priv_nh.param("grid_dimensions", grid_dim_, 200);//64
  priv_nh.param("height_threshold", height_diff_threshold_, 0.31);
                                    ...
                                    size_t &empty_count)
{
  ...
  //add 1706
  // Localization Data
  potentialMultiArray.data.clear();
...
// ed: velodyne_points 토픽을 섭스크라이브하는 콜백함수
...
void HeightMap::processData(const VPointCloud::ConstPtr &scan){
  ...
  // ed: 최종적으로 아래 코드로 인해 VPointCloud 메시지가 퍼블리시 된다
  if (obstacle_publisher_.getNumSubscribers() > 0)
