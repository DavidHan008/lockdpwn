    ...
    {
        publisherProxy->publisher = nh->advertise<driver_base::ConfigString>(in->topicName, in->queueSize, in->latch);
    }
    else if(in->topicType == "driver_base/ConfigValue")
    {
        publisherProxy->publisher = nh->advertise<driver_base::ConfigValue>(in->topicName, in->queueSize, in->latch);
    }
    else if(in->topicType == "driver_base/SensorLevels")
    {
        publisherProxy->publisher = nh->advertise<driver_base::SensorLevels>(in->topicName, in->queueSize, in->latch);
    }
