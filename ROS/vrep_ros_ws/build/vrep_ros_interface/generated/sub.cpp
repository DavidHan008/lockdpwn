    ...
    else if(in->topicType == "driver_base/ConfigString")
    {
        subscriberProxy->subscriber = nh->subscribe<driver_base::ConfigString>(in->topicName, in->queueSize, boost::bind(ros_callback__driver_base__ConfigString, _1, subscriberProxy));
    }
    else if(in->topicType == "driver_base/ConfigValue")
    {
        subscriberProxy->subscriber = nh->subscribe<driver_base::ConfigValue>(in->topicName, in->queueSize, boost::bind(ros_callback__driver_base__ConfigValue, _1, subscriberProxy));
    }
    else if(in->topicType == "driver_base/SensorLevels")
    {
        subscriberProxy->subscriber = nh->subscribe<driver_base::SensorLevels>(in->topicName, in->queueSize, boost::bind(ros_callback__driver_base__SensorLevels, _1, subscriberProxy));
    }
