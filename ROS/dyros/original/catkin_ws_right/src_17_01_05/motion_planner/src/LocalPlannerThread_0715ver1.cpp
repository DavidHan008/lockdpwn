...
void LocalPlannerThread::GetLookAheadPt_JW(double &lookAheadDist,double& x, double& y, double &resdist)
{
	...
	if (m_th_Err > M_PI)	m_th_Err = abs(m_th_Err - 2*M_PI);
	...
	cout << "JoonWoo Delta : " << lookAheadDist << ", TH_ERR : " << 1/m_th_Err << ", Dist_ERR : "<< turning_rad*m_th_Err*m_dist_Err << endl<< endl;


	double dist = 0;
        int lookAheadIndex = -1;
        for( int i=cIndex; i<m_LocalSplinePath.size()-1; i++ )
	//for( int i = lookAheadDist; i<m_LocalSplinePath.size()-1; i++ )
        {
