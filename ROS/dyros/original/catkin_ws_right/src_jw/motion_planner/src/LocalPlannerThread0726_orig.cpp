...
void LocalPlannerThread::GetLookAheadPt_JW(double &lookAheadDist,double& x, double& y, double &resdist)
{
	...
	double turning_rad_2 = (abs(atan2(abs(m_LocalSplinePath[cIndex][1] - m_LocalSplinePath[(lookAheadIndex-cIndex)/2+cIndex][1]), abs(m_LocalSplinePath[cIndex][0] - m_LocalSplinePath[abs(lookAheadIndex-cIndex)/2+cIndex][0])))) / 1.57;
...
//	turning_rad = sin((turning_rad)*M_PI/2);	//rev
	...
	m_dist_Err = sqrt((m_LocalSplinePath[cIndex][1] - m_LocalSplinePath[cIndex+1][1])*(m_LocalSplinePath[cIndex][1] - m_LocalSplinePath[cIndex+1][1]) + (m_LocalSplinePath[cIndex][0] - m_LocalSplinePath[cIndex+1][0])*(m_LocalSplinePath[cIndex][0] - m_LocalSplinePath[cIndex+1][0])); //JW 07.28.
...
void LocalPlannerThread::GetLookAheadPt(double lookAheadDist,double& x, double& y, double &resdist)
{
	...
	//JW 0728
	double heading = m_pos[2];//range 0 ~ M_PI*2
    ...
    :m_bThreadStop(false),init_argc(argc), init_argv(argv)
{
    ...
    m_model_jw.color.g = 0.0f;
    m_model_jw.color.b = 1.0f;
...
//JW 16.06.24.orig
	...
	{
		//printf("%lf\n",m_vel);
		GetLookAheadPt(4.0*m_vel,  x_orig,  y_orig, resdist);//JW 16.07.14.test1
		//GetLookAheadPt(0.0, x, y, resdist);//JW 16.07.14.test1
		vel_orig = 4.0*m_vel;
	}	
	//else
	//{
	//	GetLookAheadPt(6.0, x_orig,  y_orig, resdist);//JW 16.07.14.test1
	//	GetLookAheadPt(0.0, x, y, resdist);	//JW 16.07.14.test1
	//	vel_orig = 6.0;
	//}
...
//JW 16.06.24.test1
	//GetLookAheadPt_JW(delta_L, x_orig, y_orig, resdist);	//JW 16.07.14.test1
	...
	m_model_jw.pose.position.x = x_orig;
	m_model_jw.pose.position.y = y_orig;
	...
	//m_model_jw.pose.position.x = x_orig;
	//m_model_jw.pose.position.y = y_orig;
	//msgpub_Look_orig.publish(m_model_jw);
	//JW 16.07.08
	////////////////////////////////////////////////////////////////
}
