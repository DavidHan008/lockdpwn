...
//	turning_rad = sin((turning_rad)*M_PI/2);	//rev
	...
	lookAheadDist = delta_LookAhead - 0;
...
//	for( int i = lookAheadIndex; i<m_LocalSplinePath.size()-1; i++ )
		...
		double m_Lfw = sqrt((m_LookAhead_X - m_LocalSplinePath[cIndex][0])*(m_LookAhead_X - m_LocalSplinePath[cIndex][0]) + (m_LookAhead_Y - m_LocalSplinePath[cIndex][1])*(m_LookAhead_Y - m_LocalSplinePath[cIndex][1]));
		cout << "m_Lfw : " << m_Lfw << "  ,    delta_LookAhead : " << lookAheadDist;
		m_overall_rev = lookAheadDist*(lookAheadDist - m_Lfw);
...
//	turning_rad = sin((turning_rad)*M_PI/2);	//rev
	...
	//JW 16.06.24.test1 
	if(sin(m_th_Err*M_PI/2)<0.1)//line
		m_line_add +=0.1;
	else	m_line_add = 0;
    ...
    :m_bThreadStop(false),init_argc(argc), init_argv(argv)
{
    ...
    m_model_jw.color.r = 1.0f;	m_model_jw_exp.color.r = 1.0f;
