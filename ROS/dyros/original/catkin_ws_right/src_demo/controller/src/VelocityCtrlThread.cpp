		...
		, init_argc(argc), init_argv(argv), _ui(ui)
{
	...
	m_brake_min = 4.0;		//JW 07.20.

	//m_brakingCtrlVal = 5.0*0.278;	//orig
	m_brakingCtrlVal = 6.1;	//JW 07.20.
	...
	{
		{
			...
			if(m_bVelTopicUpdated)//Sub
			{
				...
				{
					if(m_targetVel > 0 && m_targetVel <= m_brakingCtrlVal)	// 0 ~ 1km/h	////////////////////////////
						// velocity control using brake pedal
					{
						m_brake = m_brake_pgain * 1.0 * vel_error;	//integral	(gain : -0.1) -> +	//JW 07.20.
						printf("%lf\n",m_brake);
						if(m_brake <= m_brake_min)	//0
							...
							m_brake = m_brake_max;
						///orig
						//if(vel_error > 0.0 )	//vel up
						//{
						//	m_accel = 0.0;
						//	//m_brake = m_brake + m_brake_pgain * 1.0 * vel_error;	//integral	(gain : -0.1) -> +	//orig
						//	printf("%lf\n",m_brake);
						//	if(m_brake <= m_brake_min)	//0
						//		m_brake = m_brake_min;
						//	else if(m_brake >= m_brake_max)	//18
					...
					{ 
						if(vel_error > 0.0 )
						{
							m_accel = m_accel + m_accel_pgain * vel_error;	//integral	(gain : 0.1) -> +
							if(m_accel >= m_accel_max)	//30km/h
								m_accel = m_accel_max;
							else if(m_accel <= m_accel_min)	//25km/h
								m_accel = m_accel_min;
						...
						{
							m_brake = m_brake + m_brake_pgain * vel_error;//integral	(gain : -0.1) -> +
							m_accel = 0.0;
							if(m_brake <= m_brake_min)	//25km/h
								m_brake = m_brake_min;
							else if(m_brake >= m_brake_max)	//30km/h
								m_brake = m_brake_max;
						}
					}
				}
