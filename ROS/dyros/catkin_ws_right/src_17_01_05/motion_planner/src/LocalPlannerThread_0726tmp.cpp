...
void LocalPlannerThread::GetLookAheadPt_JW(double &lookAheadDist,double& x, double& y, double &resdist)
{
	...
	double turning_rad_2 = (abs(atan2(abs(m_LocalSplinePath[cIndex][1] - m_LocalSplinePath[(int)(abs(lookAheadIndex+cIndex)/2)+cIndex][1]), abs(m_LocalSplinePath[cIndex][0] - m_LocalSplinePath[(int)(abs(lookAheadIndex-cIndex)/2)+cIndex][0])))) / 1.57;
...
//	turning_rad = sin((turning_rad)*M_PI/2);	//rev
	...
	//for( int i = lookAheadIndex; i<m_LocalSplinePath.size()-1; i++ )
		...
		m_err.data.push_back(Lps_turning_rad);
