...
void LocalPlannerThread::GetLookAheadPt_JW(double &lookAheadDist,double& x, double& y, double &resdist)
{
	...
	//JW 16.06.24.test1
	if(m_pos[2] > M_PI)
		heading = m_pos[2] - M_PI*2;
	else
		heading = m_pos[2];

	m_th_Err = abs(heading + (atan2((m_LookAhead_Y - m_pos[1]), (m_LookAhead_X - m_pos[0]))));
	m_th_Err *= 0.1;
	if (m_th_Err < 0.1) m_th_Err = 0.1;
	//printf("orig_th : %lf, err_th : %lf\n", heading, atan2((m_LookAhead_Y - m_pos[1]), (m_LookAhead_X - m_pos[0])));
	
	turning_rad = abs(M_PI - abs(atan2(m_LocalSplinePath[lookAheadIndex+1][1] - m_LocalSplinePath[lookAheadIndex][1], m_LocalSplinePath[lookAheadIndex+1][0] - m_LocalSplinePath[lookAheadIndex][0]))) / 3.14;
	turning_rad = turning_rad*turning_rad;
	

	m_dist_Err = abs(sqrt((m_LookAhead_X - m_pos[0])*(m_LookAhead_X - m_pos[0]) + (m_LookAhead_Y - m_pos[1])*(m_LookAhead_Y - m_pos[1])));
	...
	double dist = 0;
		...
		cout << "JW,, Delta : " << lookAheadDist << ", TH_ERR : " << m_th_Err << ", Dist_ERR : "<< m_dist_Err << endl;
