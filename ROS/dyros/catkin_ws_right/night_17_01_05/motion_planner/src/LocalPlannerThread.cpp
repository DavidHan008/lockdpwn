...
double m_th_Err = 0.0; //JW 16.07.11.
    ...
    if (m_LocalSplinePath.size() > 1)
    {
        ...
        int cIndex = -1;
        int lookIndex = -1;
        for(int i = (_cut_back/200)*100; i<m_LocalSplinePath.size(); i++)
        {
...
void LocalPlannerThread::GetLookAheadPt_JW_for(double &lookAheadDist,double& x, double& y, double &resdist)
{
    ...
    if (m_LocalSplinePath.size() > 1)
    {
        ...
        int cIndex = -1;
        int lookIndex = -1;
        for(int i = 0; i < (_cut_switch/200+1)*100; i++)
        {
            double x2 = m_LocalSplinePath[i][0] - m_pos[0];
            x2 *= x2;
            double y2 = m_LocalSplinePath[i][1] - m_pos[1];
            y2 *= y2;
            double dist = sqrt(pow(x2,2)+pow(y2,2));//distŽÂ 
            if ( dist < minDist )
            {
                minDist = dist;
                cIndex = i;
...
//fin.open("/home/dyros-vehicle/00_DATA/bag/map/map170105.txt");//17.01.05 parking for+rev
fin.open("/home/dyros-vehicle/00_DATA/bag/map/ms_parking_170105.txt");//17.01.05 parking for+rev
...
//cout <<"["<<map_cnt++ <<"] "<< X<<", "<< Y<<", "<<th<<", "<<vel << ", "<<pre_vel <<endl;
		...
		if (vel == 0 && pre_vel != 0 ) // forward 2 stop
		{
		// forward finish position
			nodeVec.push_back(Vector2d(X, Y));
			printf("[%d] X : %lf,  Y : %lf\n",map_cnt++,X,Y);
			_cut_switch = _cnt;
			m_switch_flag = -1;
		...
		if ( (vel != 0 && pre_vel != 0) && (m_switch_flag == 1 && _cnt%200 == 0)) // forward push back
		{
			nodeVec.push_back(Vector2d(X, Y));
			//printf("[%d] X : %lf,  Y : %lf\n",map_cnt++,X,Y);
		}
...
////////////////////////////////////////////////////////////////
		if ((vel != 0 && pre_vel == 0) && m_start_flag !=0) // stop 2 backward
		{
			nodeVec.push_back(Vector2d(X, Y));
			printf("[%d] X : %lf,  Y : %lf\n",map_cnt++,X,Y);
			_cut_back = _cnt;
		}
		if ( (vel != 0 && pre_vel != 0) && (m_switch_flag == -1 && (_cnt - _cut_back)%200 == 0)) //backward pushback
		{  
			nodeVec.push_back(Vector2d(X, Y));
			//printf("[%d] X : %lf,  Y : %lf\n",map_cnt++,X,Y);	
		}
...
//	if (dir_mode == 1)//m_switch_flag
	...
	else if (m_switch_flag == -1)////if (dir_mode == -1)
	{
