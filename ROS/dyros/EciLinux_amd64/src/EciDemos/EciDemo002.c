...
ECI_RESULT EciCanDemo002  ( DWORD   dwHwIndex,
                            DWORD   dwCtrlIndex);
...
ECI_RESULT EciDemo002(void)
{
  ...
  hResult = ECI002_Initialize(_countof(astcHwPara), astcHwPara);
  ECIDEMO_CHECKERROR(ECI002_Initialize);

  //*** Retrieve hardware info
  if(ECI_OK == hResult)
  {
    //*** Retrieve hardware info
    hResult = ECI002_GetInfo(dwHwIndex, &stcHwInfo);
    ECIDEMO_CHECKERROR(ECI002_GetInfo);
    if(ECI_OK == hResult)
  ...
  if(ECI_OK == hResult)
  {
    ...
    {
      //*** Start CAN Demo
      hResult =  EciCanDemo002(dwHwIndex, dwCtrlIndex);
      ECIDEMO_CHECKERROR(EciCanDemo002);
    }
    else
    {
  ...
  OS_Printf("-> Returning from ECI Demo for PC-I 04 / PCI <-\n");
                         ...
                         DWORD dwCtrlIndex)
{
  ...
  if(ECI_OK == hResult)
  {
    ...
    hResult = ECI002_CtrlOpen(&dwCtrlHandle, dwHwIndex, dwCtrlIndex, &stcCtrlConfig);
    ECIDEMO_CHECKERROR(ECI002_CtrlOpen);
  }
  ...
  if(ECI_OK == hResult)
  {
    ...
    hResult = ECI002_CtrlGetCapabilities(dwCtrlHandle, &stcCtrlCaps);
    ECIDEMO_CHECKERROR(ECI002_CtrlGetCapabilities);
    if(ECI_OK == hResult)
    {
  ...
  if(ECI_OK == hResult)
  {
    hResult = ECI002_CtrlStart(dwCtrlHandle);
    ECIDEMO_CHECKERROR(ECI002_CtrlStart);
  }
  ...
  if(ECI_OK == hResult)
  {
    ...
    for(dwIndex=0; dwIndex < dwTxMsgCount; dwIndex++)
    {
      ...
      #ifdef ECIDEMO_HWUSEPOLLINGMODE
      {
        ...
        do
        {
          hResult = ECI002_CtrlSend( dwCtrlHandle, &stcCtrlMsg, 0);
          if(ECI_OK != hResult)
            { OS_Sleep(1);}
        }while((ECI_OK != hResult) && ((OS_GetTimeInMs() - dwStartTime) < ECIDEMO_TX_TIMEOUT));
      }
      #else
        hResult = ECI002_CtrlSend( dwCtrlHandle, &stcCtrlMsg, ECIDEMO_TX_TIMEOUT);
      #endif //ECIDEMO_HWUSEPOLLINGMODE
      if(ECI_OK != hResult)
      {
        OS_Printf("Error while sending CAN Messages\n");
        ECIDEMO_CHECKERROR(ECI002_CtrlSend);
        hResult = ECI_OK;
        break;
      }
      else
      {
        ...
        hResult = ECI002_CtrlReceive( dwCtrlHandle, &dwCount, astcCtrlMsg, 0);
  ...
  if(ECI_OK == hResult)
  {
    ...
    while(dwRxTimeout >= (dwCurrentTime - dwStartTime))
    {
      ...
      #ifdef ECIDEMO_HWUSEPOLLINGMODE
      {
        ...
        {
          dwCount = 1;
          hResult = ECI002_CtrlReceive( dwCtrlHandle, &dwCount, &stcCtrlMsg, 0);
      ...
      {
        dwCount = 1;
        hResult = ECI002_CtrlReceive( dwCtrlHandle, &dwCount, &stcCtrlMsg, ECIDEMO_RX_TIMEOUT);
      }
      #endif //ECIDEMO_HWUSEPOLLINGMODE
  ...
  if(ECI_OK == hResult)
  {
    hResult = ECI002_CtrlStop(dwCtrlHandle, ECI_STOP_FLAG_NONE);
    ECIDEMO_CHECKERROR(ECI002_CtrlStop);
  }
  ...
  if(ECI_OK == hResult)
  {
    hResult = ECI002_CtrlStop(dwCtrlHandle, ECI_STOP_FLAG_RESET_CTRL);
    ECIDEMO_CHECKERROR(ECI002_CtrlStop);
  }

  //*** Close ECI Controller
  ECI002_CtrlClose(dwCtrlHandle);
