...
ECI_RESULT EciCanDemo10A  ( DWORD   dwHwIndex,
                            DWORD   dwCtrlIndex);
ECI_RESULT EciLinDemo10A  ( DWORD   dwHwIndex,
                            DWORD   dwCtrlIndex);
...
ECI_RESULT EciDemo10A(void)
{
  ...
  hResult = ECI10A_Initialize(_countof(astcHwPara), astcHwPara);
  ...
  if(ECI_OK == hResult)
  {
    ...
    {
      //*** Start LIN Demo
      hResult =  EciLinDemo10A(dwHwIndex, dwCtrlIndex);
      ECIDEMO_CHECKERROR(EciLinDemo10A);
    }
    else
    {
      //*** Ignore if no controller was found
