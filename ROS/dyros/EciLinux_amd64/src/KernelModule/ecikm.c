...
#define CAN_IB530_DEVICE_ID                 0x0013
#define CAN_IB530_SUB_DEVICE_ID             0x0013
...
#define CAN_IB630_DEVICE_ID                 0x0016
#define CAN_IB630_SUB_DEVICE_ID             0x0016
    ...
    if(status && (NULL != pDeviceData->addmemvadd))
    {
      ...
      for(idx=0; idx<PCIE_ALTERA_LCR_A2P_MAILBOX_COUNT; idx++)
      {
        if(irqsrc & 0x01)
        {
          ...
          if(0 != idx)
            { __sync_lock_test_and_set(&pShadow->MBX[idx].adwA2PMailBox, *pMailbox); } // replace completely (assign)
          else
            { __sync_fetch_and_or(&pShadow->MBX[idx].adwA2PMailBox, *pMailbox); } // Add new flags (or)
        }
        irqsrc >>= 1;
        pMailbox++;
      }
      ...
      __sync_fetch_and_or(&pShadow->dwInterruptStatus, status);  // Add new flags (or)
    }
...
static int probe(struct pci_dev * dev, const struct pci_device_id *id)
{
  ...
  // kernel versions prior 2.6.28 have to use device_create_drvdata() here
    ...
    case IOCTL_GETDMAINFO:
    {
      ...
      if(0 == ret)
      {
        switch(sDmaInfo.lDmaIndex)
        {
          ...
          default:
          {
            printk(KERN_WARNING "ECI: IOCTL_GETDMAINFO - unknown DMA index %lu!\n", sDmaInfo.lDmaIndex);
            ret = -EINVAL;
          }
        }
      }
...
int open (struct inode *inode, struct file * file)
{
  ...
  #ifdef __DEBUG__
    printk("ECI: %s inode: 0x%px, file: 0x%px\n", __func__, inode, file);
  #endif //__DEBUG__
...
///////////////////////////////////////////////////////////////////////////////
/**
This is the flush function for our char device. It will be called when a userspace application
...
int flush( struct file * file, fl_owner_t id)
{
  ...
  #ifdef __DEBUG__
    printk("ECI: %s file: 0x%px name %s usage_count %d \n", __func__,  file, pDeviceData->name, pDeviceData->usage_count);
  #endif //__DEBUG__

  if(!pDeviceData->usage_count)
    return 0;
...
int isa_add(unsigned long memadd, unsigned long memlen, unsigned int irq, int devicetype)
{
  ...
  if(check_mem_region(pDeviceData->memadd, pDeviceData->memlen))
  {
  ...
  request_mem_region( pDeviceData->memadd, pDeviceData->memlen, "ECI ISA Memory" );
  pDeviceData->memvadd = ioremap_nocache( pDeviceData->memadd, pDeviceData->memlen );
...
int isa_scan_iomem(void)
{
  ...
  for(; addr <= ECI_ISA_ADDRESS_END; addr += step)
  {
    ...
    else
    {
      ...
      request_mem_region(addr, len, "ECI ISA Memory");
