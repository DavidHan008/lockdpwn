...
$ attach() ==> 데이터셋의 각 변수들을 불러온다
	
		Mileage 데이터셋 변수를 불러옵니다
		$ attach(Mileage)
...
$ pqdr 확률분포 관련 함수 
	...
	$ rnorm() ==> 정규분포를 랜덤으로 만드는 함수
			평균이 3이고 표준편차가 1인 정규분포값을 랜덤으로 20개 생성합니다
			$ rnorm(20,3,1)
...
$ locator ==> 그래프에 마우스로 찍는 위치를 반환해주는 함수

		그래프상에 2개의 빨간색 점을 찍어서 좌표를 반환한다
		$ c <- localtor(2); print(c)
...
=======================================================================
# 2016.2학기 마이크로프로세서 중간기말 성적. 난 [93,71]
