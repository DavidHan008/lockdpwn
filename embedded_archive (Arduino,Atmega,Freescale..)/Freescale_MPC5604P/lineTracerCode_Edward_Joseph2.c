...
#define LIMITEDMOTORMAXSPEED 703
...
void Sensing()
{
	...
	if(lastTracesensorValue != tracesensor.sen.R)
	{
		...
		if(BrakeTimer)
		{
			ErrorVelocity = SensorErrorAmount/BrakeTimer;
			if(ErrorVelocity <= 5 && SensorErrorAmount <= 30)
			{
				DisturbanceAmount++;
			}
		}
...
void LineTracing()
{
	switch(tracesensor.sen.R)
	{
		case 0b1111:
			...
			{
				setPointDCMotor = ADCspeedController + 2;
			}
			
			// 특정시간동안 0b1111만 읽힌경우 밖에있는걸로 간주하고 감속한다
			if(IsInOutLine >= 50)
			{
				setPointDCMotor = ADCspeedController + 18;
			}
			IsMiddle = FALSE;
			IsMiddle2 = FALSE;
			break;
		...
		case 0b1110: 							
			...
			if(ShakingTime)
			{
				setPointDCMotor = ADCspeedController + 16;
			}
			else
			{
				setPointDCMotor = ADCspeedController + 7;
			}
			break;
		case 0b0111: 						
			if(SuperBrakeTime && Brakeflag1)
	...
	if(sensorValues[0] == 1 || sensorValues[3] == 1)
	{
		BoostTime++;
	}
	else
	...
	{
		motorControl += DisturbanceAmount/6;
	}

	if(BoostTime >= 30000)
	{
		motorControl = LIMITEDMOTORMAXSPEED;
	}
