...
void CalculateInputFromSensor()
{
	...
	else if(posLeft != 10)   // 0b1110, 0b1100, 0b1101 의 경우
	{
		...
		if(ShakingTime || DisturbanceAmount >= 5)
		{
			input = 5;
		}
	}
...
void LineTracing()
{
	switch(tracesensor.sen.R)
	{
		case 0b1111:
			...
			{
				setPointDCMotor = ADCspeedController + 4;
			}
			break;
		...
		case 0b1001:
			...
			if(ShakingTime)
			{
				setPointDCMotor = ADCspeedController + 25;
			}
			else
			{
	...
	if(DisturbanceAmount >= 10 && DisturbanceAmount <= 50)
	{
		motorControl += DisturbanceAmount/5;
	}
