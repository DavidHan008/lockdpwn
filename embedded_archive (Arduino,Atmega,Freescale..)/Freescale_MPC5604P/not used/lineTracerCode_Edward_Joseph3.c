...
#define PIDMINDCMotor LIMITEDMOTORMAXSPEED-5
#define PIDMAXDCMotor LIMITEDMOTORMAXSPEED+25
...
void CalculateInputFromSensor()
{
	...
	if((posLeft != 10) && (posRight != 10)) // 0b1001, 0b0000의 경우
	{
		input = 0;
	}
	else if(posLeft != 10)   // 0b1110, 0b1100, 0b1101 의 경우
	{
		input = posLeft*2 + sum;  // 0b1110 : (3-2)*2 + 1 = 3
								   // 0b1100 : (2-2)*20 + 2 = 2
								   // 0b1101 : (2-2)*20 + 1 = 1
	}
	else if(posRight != 10)  // 0b0111, 0b0011, 0b1011 의 경우
	{
		input = posRight*2 - sum; // 0b0111 : (0-1)*2 -1 = -3
		  						   // 0b0011 : (1-1)*2 -2 = -2
		 						   // 0b1011 : (1-1)*2 - 1 = -1
	}
}

void PIDComputeforServo(double *setpointPID, double *inputPID, double *outputPID, double kp, double ki, double kd, double Min, double Max)
{
	outMin = Min;
    outMax = Max;
...
void LineTracing()
{
	switch(tracesensor.sen.R)
	{
		...
		case 0b0000:    							// 모두 검정색일 때
			setPointDCMotor = ADCspeedController + 35;
			break;
		case 0b1001:
			...
			break;
		case 0b1011:
			setPointDCMotor = ADCspeedController + 15;
			break;
		case 0b1101:
			setPointDCMotor = ADCspeedController + 15;
			break;
...
{
	PIDComputeforServo(&setPointServo, &input, &servoControl, Kp, Ki, Kd, PIDMIN, PIDMAX);
	PIDComputeforDCMotor(&setPointDCMotor, &inputDCMotor, &motorControl, Kp2, Ki2, Kd2, PIDMINDCMotor, PIDMAXDCMotor);
...
{
	ShakeAmount = 0;
	PIT.CH[1].TFLG.B.TIF = 1;  // Clear PIT0 flag
}
...
int main(void)
{
	...
	INTC_InstallINTCInterruptHandler(TimerforRecognizeShake,60,6);
