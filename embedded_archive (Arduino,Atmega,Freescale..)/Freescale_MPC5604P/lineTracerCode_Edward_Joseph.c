...
#define LIMITEDMOTORMAXSPEED 706
#define PIDMAXDCMotor LIMITEDMOTORMAXSPEED+16
...
void LineTracing()
{
	switch(tracesensor.sen.R)
	{
		case 0b1111:
			if(ShakingTime)
			{
				setPointDCMotor = ADCspeedController + 14;
			}
			else
			{
				setPointDCMotor = ADCspeedController + 12;
			}
	...
	{
		motorControl += DisturbanceAmount/7;
	}
...
void DCServoMotorControlPWM()
{
	...
	if(SensorErrorAmount >= 30)
	{
		motorControl = LIMITEDMOTORMAXSPEED + 10;
	}
