...
void track(int, void*) {
	...
	if (contours.size() > 0) {
		...
		vector<vector<Point> >hullPoint(contours.size()); //elin hareketine g�re eli �evreleyen �okgen	
		vector<vector<Vec4i> > defects(contours.size()); //parmak u�lar�ndaki ye�il noktalar..multi dimensional matrix
		vector<vector<Point> >defectPoint(contours.size()); //point olarak parmak ucu noktalar�n� x,y olarak tutuyor
		vector<vector<Point> >contours_poly(contours.size()); //eli �evreleyen hareketli dikd�rtgen		
