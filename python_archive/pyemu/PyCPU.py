...
class PyCPU:
    ...
    # Convenience function for setting shift operation flags
    def set_shift_flags(self, result, size):
        if size == 1:
            self.SF = int(((result & 0xff) >= 0x80))
        elif size == 2:
            self.SF = int(((result & 0xff00) >= 0x8000))
        elif size == 4:
            self.SF = int(((result & 0xff000000) >= 0x80000000))
        else:
            self.SF = 0
        
        # Yea exactly...
        self.PF = int(not "".join(map(lambda x:str((result >> x) & 1), range(8 - 1, -1, -1))).count('1') & 0x1)
    ...
    # Convenience function for setting arithmetic operation flags         
    def set_arithmetic_flags(self, op1, op2, result, size, direction=None):
        if size == 1:
            mask = 0x80
        elif size == 2:
            mask = 0x8000
        ...
        if size == 1:
            self.SF = int(((result & 0xff) >= mask))
        elif size == 2:
            self.SF = int(((result & 0xff00) >= mask))
        elif size == 4:
            self.SF = int(((result & 0xff000000) >= mask))
        else:
            self.SF = 0
        
        if size == 1:
            self.CF = int(((op1 ^ result) & mask) >> 7)
        elif size == 2:
            self.CF = int(((op1 ^ result) & mask) >> 15)
        elif size == 4:
            self.CF = int(((op1 ^ result) & mask) >> 31)
        else:
            self.CF = 0
        
        if direction == "add":
            if mask:         
                self.OF = int((((~((op1) ^ (op2)) & ((op2) ^ (result))) & (mask)) != 0))
            else:
                self.OF = 0
        elif direction == "sub":
            if mask:
                self.OF = int((((((op1) ^ (op2)) & ((op1) ^ (result))) & (mask)) != 0))
            else:
                self.OF = 0
        else:
            self.OF = 0
    ...
    def MOV(self, instruction):
        ...
        #A1 MOV EAX,moffs32* Move doubleword at (seg:offset) to EAX
            ...
            if instruction.fs_override():
                ...
                op2value = self.get_memory(baseaddress + offset, size)
                self.set_register(0, op2value, size)
            else:
                op2value = self.get_memory_address(instruction, 2, size)
                offset = op2.displacement
                op2valuederef = self.get_memory(op2value + offset, size)
                
                self.set_register(0, op2valuederef, size)
        ...
        #A3 MOV moffs32*,EAX Move EAX to (seg:offset)
            ...
            if instruction.fs_override():
                ...
                self.set_memory(baseaddress + offset, op2value, size)
            else:
        ...
        #B0+ rb MOV r8,imm8 Move imm8 to r8
            ...
            self.set_register8(op1value, op2value)
    ...
    def RET(self, instruction):
        ...
        #C2 iw RET imm16 Valid Valid Near return to calling procedure and pop imm16 bytes from stack.
            ...
            eip = self.get_memory32(self.get_register32("ESP"))
            esp = self.get_register32("ESP") + op1value
