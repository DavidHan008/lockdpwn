				...
				case ID_CAPTURE_START:
					{
						...
						//Modify the Icon State
						if(!Shell_NotifyIcon(NIM_MODIFY,&nid))
							MessageBox(NULL,"Unable to Set Notification Icon"
								   ,"Error"
				...
				case ID_CAPTURE_STOP:
					{
						...
						strcpy(nid.szTip
						       ,"Screen Capture Paused - Double Click to Resume");

						if(!Shell_NotifyIcon(NIM_MODIFY,&nid))				//Modify the Icon State
							MessageBox(NULL
								   ,"Unable to Set Notification Icon"
								   ,"Error"
								   ,MB_ICONINFORMATION|MB_OK);
		...
		case WM_INITMENU:
			{
				...
				EnableMenuItem(hMenu
					       ,ID_FILE_SHOWWINDOW
					       ,IsWindowVisible(hWnd)?MF_GRAYED:MF_ENABLED|MF_BYCOMMAND);

				EnableMenuItem(hMenu
					       ,ID_FILE_HIDEWINDOW
					       ,IsWindowVisible(hWnd)?MF_ENABLED:MF_GRAYED|MF_BYCOMMAND);

				EnableMenuItem(hMenu
					       ,ID_FILE_CAPTURESCREENSHOT
					       ,gbCapturing?MF_GRAYED|MF_BYCOMMAND:MF_ENABLED);

				EnableMenuItem(hMenu
					       ,ID_FILE_STARTCAPTURINGTOAVI,
					       gbCapturing?MF_GRAYED|MF_BYCOMMAND:MF_ENABLED);

				EnableMenuItem(hMenu
					       ,ID_FILE_PAUSECAPTURINGTOAVI
					       ,gbCapturing?MF_ENABLED:MF_GRAYED|MF_BYCOMMAND);
